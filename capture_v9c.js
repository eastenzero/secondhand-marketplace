...
(async () => {
    ...
    try {
        ...
        await page.screenshot({ path: path.join(OUT, 'secondhand_photo_hero_v9c.png'), fullPage: false });
        console.log(' -> secondhand_photo_hero_v9c.png');

        console.log('2. Capturing Item List (scrolling down)...');
        await page.evaluate(() => window.scrollBy(0, window.innerHeight / 2));
        await new Promise(r => setTimeout(r, 1000));
        await page.screenshot({ path: path.join(OUT, 'secondhand_photo_list_v9c.png'), fullPage: false });
        console.log(' -> secondhand_photo_list_v9c.png');
        const imgSrcs = await page.evaluate(() => Array.from(document.querySelectorAll("img")).map(img => img.src).filter(src => src.includes("photo")));
        console.log("   --- DOM IMG SRC PROOF ---");
        imgSrcs.forEach(src => console.log("   IMG: " + src));
        console.log("   -------------------------");
        ...
        await page.screenshot({ path: path.join(OUT, 'secondhand_photo_empty_v9c.png'), fullPage: false });
        console.log(' -> secondhand_photo_empty_v9c.png');
        ...
        await new Promise(r => setTimeout(r, 2000));
        await page.screenshot({ path: path.join(OUT, 'secondhand_photo_detail_v9c.png'), fullPage: false });
        console.log(' -> secondhand_photo_detail_v9c.png');
        ...
        if (toggled) {
            ...
            await new Promise(r => setTimeout(r, 2000));
            // Save compare image
            await page.screenshot({ path: path.join(OUT, 'secondhand_cartoon_compare_v9c.png'), fullPage: false });
            console.log(' -> secondhand_cartoon_compare_v9c.png');

            // Save webp style toggle
            await page.screenshot({
                path: path.join(OUT, 'secondhand_style_toggle_v9c.webp'),
                type: 'webp',
                quality: 80,
            });
            console.log(' -> secondhand_style_toggle_v9c.webp');
        } else {
